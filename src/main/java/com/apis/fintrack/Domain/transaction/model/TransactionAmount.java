...
/**
 * Value Object que representa el monto de una transacciÃ³n.
 * 
 * Reglas de negocio:
 * - El valor NO puede ser cero
 * - Los gastos (EXPENSE) se almacenan con valor NEGATIVO
 * - Los ingresos (INCOME) se almacenan con valor POSITIVO
 * 
 * CaracterÃ­sticas:
 * - Inmutable
...
public final class TransactionAmount {
    ...
    /**
     * Crea un TransactionAmount segÃºn el tipo de transacciÃ³n.
     * 
     * @param amount el monto (siempre positivo en entrada)
     * @param type el tipo de transacciÃ³n
     * @return TransactionAmount con el signo correcto segÃºn el tipo
     */
     ...
     * 
     * @param storedValue el valor tal como estÃ¡ almacenado (ya con signo)
     * @return una nueva instancia de TransactionAmount
     */
    public static TransactionAmount fromStorage(BigDecimal storedValue) {
        ...
        if (storedValue.compareTo(BigDecimal.ZERO) == 0) {
            throw new IllegalArgumentException("El monto de una transacciÃ³n no puede ser cero");
        }
    ...
    /**
     * Obtiene el tipo de transacciÃ³n basado en el signo.
     * 
