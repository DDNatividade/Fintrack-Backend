...
public class TransactionPersistenceMapper {
    ...
    public TransactionJPAEntity toJpaEntity(Transaction domainTransaction) {
       Objects.requireNonNull(domainTransaction, "La entidad de dominio no puede ser nula");
        ...
        jpaEntity.setCategory(domainTransaction.getCategoryValue());
        jpaEntity.setIncome(domainTransaction.isIncome());
        
        // Buscar el usuario
        UserJPAEntity userEntity = userRepository.searchById(domainTransaction.getUserIdValue())
            .orElseThrow(() -> new IllegalArgumentException(
                "No existe usuario con ID: " + domainTransaction.getUserIdValue()
            ));
