package com.apis.fintrack.application.userCasesImpl.subscription;
...
public class UpdateSubscriptionUseCaseImpl implements UpdateSubscriptionUseCase {
    ...
    public Subscription update(UpdateSubscriptionCommand command) {
        var maybeUser = com.apis.fintrack.domain.user.model.User.createWithId(command.subscriptionId() == null ? null : new com.apis.fintrack.domain.user.model.UserId(command.subscriptionId()));
        ...
        command.reactivate().ifPresent(reactivate -> {
            if (reactivate) subscription.reactivate();
        });
